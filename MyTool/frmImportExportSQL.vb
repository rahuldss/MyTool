...
Public Class frmImportExportSQL
    ...
    Dim ConSourceSql As New SqlConnection
    ...
    Private Sub ImportExport_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            'txtFile.Text = strDBDetailsSplit(0)
            txtServerSRC.Text = strDBDetailsSplitSRC(1)
            txtUIDSRC.Text = strDBDetailsSplitSRC(2)
            txtPasswordSRC.Text = strDBDetailsSplitSRC(3)
            txtDatabaseSRC.Text = strDBDetailsSplitSRC(4)
    ...
    Private Sub cmdOKSRC_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdOKSRC.Click
        Try
            '***SAVE DATABASE SETTINGS SOURCE***
            MyCLS.clsCOMMON.SaveSettings("", txtServerSRC.Text, txtUIDSRC.Text, txtPasswordSRC.Text, txtDatabaseSRC.Text, True)

            OpenConnectionSource()

            FillListWithTablesFromSource()

            cboTableSource.Focus()
        Catch ex As Exception
            MyCLS.clsHandleException.HandleEx(ex, System.Reflection.MethodBase.GetCurrentMethod.ToString())
        End Try
    End Sub
    ...
    Private Sub cmdOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdOK.Click
        Try
            ...
            MyCLS.clsCOMMON.SaveSettings("", txtServer.Text, txtUID.Text, txtPassword.Text, txtDatabase.Text, False)
    ...
    Function CreateConnString(Optional ByVal isSQL As String = "") As String
        ...
        Return strConnStr
    End Function
    Function CreateConnStringSRC(Optional ByVal isSQL As String = "") As String
        Dim strConnStr As String
        If chkDBFile.Checked = True Then
            strConnStr = txtServer.Text
        Else
            If Len(isSQL) > 0 Then
                strConnStr = "UID=" & txtUIDSRC.Text & ";Password=" & txtPasswordSRC.Text & ";Data Source=" & txtServerSRC.Text & ";Initial Catalog=" & txtDatabaseSRC.Text & ";"
            Else
                strConnStr = "UID=" & txtUIDSRC.Text & ";Password=" & txtPasswordSRC.Text & ";Data Source=" & txtServerSRC.Text & ";Initial Catalog=" & txtDatabaseSRC.Text & ";Provider=SQLOLEDB.1;"
            End If

            If Len(txtPasswordSRC.Text) = 0 Then
                strConnStr = strConnStr.Replace("Password=", "Integrated Security=SSPI")
            End If
        End If
        Return strConnStr
    ...
    Private Sub OpenConnectionDest()
        ...
        End Try
    End Sub
    Private Sub OpenConnectionSource()
        Try
            Try
                ConSource.Close()
            Catch ex As Exception
            ...
            End Try
            'Con.ConnectionString = "Server=.;Initial Catalog=jpbrothers;uid=sa;password=sa123;" 'Integrated Security=SSPI"
            ConSource.ConnectionString = CreateConnStringSRC()
            ConSource.Open()

            ConSourceSql.ConnectionString = CreateConnStringSRC("SQL")
            ConSourceSql.Open()
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    ...
    Public Sub GetDataFromSourceSQL()
        Try
            '---------------GET RECORDS TO INSERTS--------------------
            Dim ds As DataSet
            Dim sb As New StringBuilder
            Dim intTotalRows As Int16 = 0
            Dim intRow As Integer
            '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            Dim isID As Boolean = False
            Dim id As Int32 = 10096
            Dim MaxID As Int32 = 2
            If Not IsNumeric(txtMaxID.Text) Then
                MsgBox("Enter Numeric value in MaxID.")
                Exit Sub
            End If
            If txtMaxID.Text.Length > 0 And Convert.ToInt32(txtMaxID.Text) > 0 Then
                isID = True
                MaxID = txtMaxID.Text
            End If
            ...
            While id < MaxID
                '1.
                Try
                    lblMSG.Text = "Fetching Records to INSERT... : " & id
                    Me.Refresh()
                    Dim sQ As String = ""
                    If isID Then
                        sQ = CreateXlsQry(Replace(cboTableSource.Text, "'", ""), id)
                    Else
                        sQ = CreateXlsQry(Replace(cboTableSource.Text, "'", ""), 0)
                    End If
                    ds = New DataSet
                    Dim DataAdapter As New SqlDataAdapter(sQ, ConSourceSql)
                    DataAdapter.SelectCommand.CommandTimeout = 10000
                    DataAdapter.Fill(ds, cboTableSource.Text)
                    intTotalRows = ds.Tables(cboTableSource.Text).Rows.Count
                    If intTotalRows = 0 Then
                        lblMSG.Text = "No Record Fetched...!"
                    Else
                        lblRecords.Text = "0 / " & intTotalRows
                    End If
                Catch ex As Exception
                    'MsgBox(ex.Message)
                    'MyCLS.clsHandleException.HandleEx(ex, System.Reflection.MethodBase.GetCurrentMethod.ToString())
                    ds = Nothing
                End Try
                '--------For Inserting Into Destination-----------------
                MyCLS.clsCOMMON.SetCon(ConDestSql)
                If ds IsNot Nothing Then
                    If ds.Tables IsNot Nothing And ds.Tables.Count > 0 Then
                        If ds.Tables(cboTableSource.Text).Rows IsNot Nothing Then
                            If ds.Tables(cboTableSource.Text).Rows.Count > 0 Then
                                InsertData(ds)
                            End If
                        End If
                    End If
            ...
            End While
            '---------------GENERATE INSERTS--------------------
            MsgBox("Data INSERTED to SQL database successfully!")
        Catch ex As Exception
            MsgBox(ex.Message)
        ...
        End If
    End Sub
    Public Sub InsertData(ds As DataSet)

        Dim intTotalRows As Int16 = 0
        Dim strColName As String = ""
        Dim strOutParamValues As String()
        Dim Packet As New MyCLS.TransportationPacket
        Dim clsESPSql As New MyCLS.clsExecuteStoredProcSql

        lblMSG.Text = "Starting INSERTING Records..."
        intTotalRows = ds.Tables(cboTableSource.Text).Rows.Count

        Dim strINSERTQ As String = CreateINSERTQry(cboTableSource.Text)

        '***ALL ROWS***
        For intRow = 0 To intTotalRows - 1
            lblRecords.Text = intRow + 1 & " / " & intTotalRows
            'lblMSG.Text = ds.Tables(cboTableSource.Text).Rows(intRow)(0).ToString()
            ...
            Try
                ...
                For i As Int16 = 0 To LstColumnsDest.Items.Count - 1 '- 2        '***NOOFCOLS
                    If LstColumnsDest.GetItemChecked(i) = True Then
                        strColName = LstColumnsSource.Items(i).ToString
                        If strColName.Contains("Date") Then
                            objParamList.Add(New SqlParameter("@" & strColName, ds.Tables(cboTableSource.Text).Rows(intRow)(strColName)))
                        Else
                            objParamList.Add(New SqlParameter("@" & strColName, ds.Tables(cboTableSource.Text).Rows(intRow)(strColName).ToString))
                        End If
                    End If
                Next
                objParamListOut.Add(New SqlParameter("@@RETURN", SqlDbType.Decimal))

                Packet.CommandText = strINSERTQ     '***INSERT QUERY
                lblMSG.Text = "Inserting Records... No. : " & intRow + 1 & " Of " & intTotalRows
                strOutParamValues = clsESPSql.ExecuteSPNonQueryOutPutCMDText(Packet.CommandText, objParamList, objParamListOut, Packet.MessageId)
                Packet.MessageResultset = strOutParamValues

                If Packet.MessageId > -1 Then
                    '***UPDATE LOCAL RECORD'S STATUS AS UPLOADED (TRUE/1)***
                    'Try
                    '    Dim cmd As New SqlCommand
                    '    lblMSG.Text = "Updating Status..."
                    '    cmd.CommandText = "UPDATE RECORDS SET STATUS=1 WHERE ID=" & ds.Tables(cboTableSource.Text).Rows(intRow)("id").ToString()
                    '    If ConSourceSql.State <> ConnectionState.Open Then
                    '        ConSourceSql.Open()
                    '    End If
                    '    cmd.Connection = ConSourceSql
                    '    cmd.ExecuteNonQuery()
                    'Catch ex As Exception
                    '    MyCLS.clsHandleException.HandleEx(ex, System.Reflection.MethodBase.GetCurrentMethod.ToString())
                    'End Try
                    lblMSG.Text = "Inserted Records... " & intRow + 1 & " Of " & intTotalRows
                End If
            Catch ex As Exception
    ...
    Public Sub GetDataFromSource()
        Try
            ...
            Dim sQ As String = CreateXlsQry(Replace(cboTableSource.Text, "'", ""), 0)
    ...
    Function CreateXlsQry(ByVal TableName As String, ByVal ID As Integer) As String
        Dim strQ As String = ""
        Try
            'MyCLS.clsCOMMON.SetCon(ConSourceSql)
            'strQ = "SELECT Top " & MyCLS.clsCOMMON.fnQuerySelect1Value("Select Count(*) From [" & TableName & "]", "Number") & " "
            strQ = "SELECT "
            For i As Int16 = 0 To LstColumnsSource.Items.Count - 1 '- 2  '***NOOFCOLS
            ...
            Next
            If ID > 0 Then
                strQ = Mid(strQ, 1, Len(strQ) - 1) & " From [" & TableName & "] Where ID = " & ID
            Else
                strQ = Mid(strQ, 1, Len(strQ) - 1) & " From [" & TableName & "] Order By 1"
            End If
    ...
    Function CreateINSERTQry(ByVal TableName As String) As String
        Dim strQ As String = ""
        Try
            'MyCLS.clsCOMMON.SetCon(ConSourceSql)
            'strQ = "SELECT Top " & MyCLS.clsCOMMON.fnQuerySelect1Value("Select Count(*) From [" & TableName & "]", "Number") & " "
            If chkIdentityRequired.Checked = True Then
                strQ = "SET IDENTITY_INSERT " & TableName & " ON INSERT INTO " & TableName & " ("
            Else
                strQ = "INSERT INTO " & TableName & " ("
            End If
            For i As Int16 = 0 To LstColumnsSource.Items.Count - 1 '- 2      '***NOOFCOLS
                If LstColumnsSource.GetItemChecked(i) = True Then
                    strQ = strQ & "[" & LstColumnsSource.Items(i) & "],"
            ...
            strQ = Mid(strQ, 1, Len(strQ) - 1)
            If chkIdentityRequired.Checked = True Then
                strQ = strQ & ") SET IDENTITY_INSERT " & TableName & " OFF"
            Else
                strQ = strQ & ") "
            End If
    ...
    Private Sub chkSelectAll_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkSelectAll.CheckedChanged
        If chkSelectAll.Checked = True Then
            ...
            MyCLS.clsControls.prcListCheckAll(LstColumnsSource)
        Else
            MyCLS.clsControls.prcListUnCheckAll(LstColumnsDest)
            MyCLS.clsControls.prcListUnCheckAll(LstColumnsSource)
        End If
